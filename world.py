...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = tuple(
                (random.randrange(1 + SNAKE_SIZE, self.size[0] - 1 - int(SNAKE_SIZE)),  # -------------
                 random.randrange(1 + SNAKE_SIZE, self.size[1] - 1 - int(SNAKE_SIZE))))  # -------------
    ...
    def init_food(self):
        ...
        for i in range(1, len(self.world) - 1):
            for j in range(1, len(self.world[0]) - 1):
                def isSnake(t):
                    for k in self.snake.blocks:
                        if k == t: return True
                    else:
                        return False

                if not isSnake(tuple((i, j))):
                    setOfPositions.add(tuple((i, j)))
        self.current_available_food_positions = setOfPositions
        if not self.custom:
            # Choose a random position from available now
            rand = random.sample(setOfPositions, 1)
            chosen_position = rand[0]
        else:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.reach_bounds():
                self.snake.alive = False
            # Check if snake eats itself
            elif self.eats_itself():
                self.snake.alive = False
    ...
    def eats_itself(self):
        for i in range(1, len(self.snake.blocks)):
            if self.snake.blocks[0] == self.snake.blocks[i]:
                return True
        else:
            return False
    ...
    def reach_bounds(self):
        for k in range(len(self.world[0])):
            if (self.snake.blocks[0] == tuple((0, k))) or (self.snake.blocks[0] == tuple((len(self.world) - 1, k))) or \
                    (self.snake.blocks[0] == tuple((k, 0))) or (
                    self.snake.blocks[0] == tuple(((len(self.world) - 1), -1))):
                return True
        return False
